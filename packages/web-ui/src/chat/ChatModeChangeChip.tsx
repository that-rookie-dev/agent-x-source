...
export function ChatModeChangeChip({ from, to }: { from: string; to: string }) {
  ...
  const chipColor = isHyperdrive ? '#ff00ff' : to === 'Plan' ? colors.accent.blue : colors.accent.orange;
    ...
    <Box sx={{ display: 'flex', justifyContent: 'center', my: 1.5, animation: 'agentx-fadeIn 0.25s ease-out' }}>
      <Chip
        ...
        sx={{
          ...
          bgcolor: `${alphaColor(chipColor, '12')}`,
          border: `1px solid ${alphaColor(chipColor, '30')}`,
          borderRadius: '10px',
          color: chipColor,
