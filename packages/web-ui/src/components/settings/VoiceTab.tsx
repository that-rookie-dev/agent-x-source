...
export function VoiceTab({ value, onChange }: VoiceTabProps) {
  ...
  return (
    <Box>
      ...
      <SettingsCard
        title="Voice module"
        subtitle="Master switch for all voice features in the app"
        accent={settingsTheme.accent.hud}
        active={voiceConfig.enabled}
      >
        <Box sx={{ display: 'flex', gap: 1.5 }}>
          <Box
            ...
            sx={{
              ...
              p: 1.5,
              borderRadius: 1,
              border: `1.5px solid ${voiceConfig.enabled ? settingsTheme.accent.hud : settingsTheme.border.default}`,
              bgcolor: voiceConfig.enabled ? `${alphaColor(settingsTheme.accent.hud, '0a')}` : 'transparent',
              cursor: 'pointer',
              transition: 'border-color 0.15s, background-color 0.15s',
              '&:hover': !voiceConfig.enabled ? { borderColor: `${settingsTheme.accent.hud}88` } : {},
            }}
          >
          ...
          >
            ...
            <Typography sx={{ ...settingsHelperSx, fontSize: '0.58rem' }}>
              Hide all voice controls and inputs.
            </Typography>
          </Box>
        </Box>
        <Typography sx={{ ...settingsHelperSx, mt: 1.5 }}>
          {voiceConfig.enabled
            ...
            : 'Voice is off — all voice icons and controls are hidden across Agent-X.'}
          {engine === 'stt_llm_tts'
            ? (!kitReady && ' Deploy the voice kit first to keep voice on.')
            : (!xaiConfigured && ' Connect your xAI API key first to keep voice on.')}
        </Typography>
      </SettingsCard>
            ...
            xaiConfigured ? (
              ...
              <Box sx={{ mb: 2 }}>
                <Typography sx={{ ...settingsHelperSx, fontSize: '0.62rem', mb: 0.5 }}>
                  Add your xAI API key in the Voice engine section above, then validate it.
                </Typography>
              </Box>
            )
          ) : kitReady ? (
