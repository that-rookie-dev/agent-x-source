...
export function PermissionsTab({ value, onChange }: PermissionsTabProps) {
  ...
  const resetToDefault = async () => {
    setResetting(true);
    try {
      await settingsPermissions.update({});
      onChange({});
  ...
  const overriddenCount = useMemo(
    () => tools.filter((t) => t.id in permissions).length,
    [tools, permissions],
  );
  ...
  const filterChips: Array<{ id: typeof filter; label: string; count: number }> = [
    ...
    { id: 'allow', label: 'Allow', count: tools.filter((t) => effectiveDecision(t) === 'allow').length },
    { id: 'ask', label: 'Ask', count: tools.filter((t) => effectiveDecision(t) === 'ask').length },
    { id: 'deny', label: 'Deny', count: tools.filter((t) => effectiveDecision(t) === 'deny').length },
  ];
        ...
        {!loading && grouped.map(([category, catTools]) => (
          <Box key={category} sx={{ mb: 2 }}>
            <Typography sx={{
              ...
              {categoryLabel(category)} ({catTools.length})
            </Typography>
