...
export type ParticlePhase = 'disabled' | 'idle' | 'recording' | 'thinking' | 'speaking';
...
const PHASE_COLORS: Record<ParticlePhase, { r: number; g: number; b: number }> = {
  disabled: { r: 80, g: 80, b: 90 },
  idle: { r: 59, g: 130, b: 246 },     // blue
  recording: { r: 34, g: 197, b: 94 },  // green
  thinking: { r: 249, g: 115, b: 22 },  // orange
  speaking: { r: 168, g: 85, b: 247 },  // purple
};
...
export function VoiceParticleField({
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      const glowAlpha = isActive ? 0.08 : 0.03;
      ...
      for (const p of particles) {
        ...
        switch (currentPhase) {
          ...
          case 'speaking': {
            // Radiate outward in waves
            const waveSpeed = 30;
            const waveRadius = ((time * waveSpeed + p.maxLife * 100) % 120);
            p.x = cx + Math.cos(p.angle) * (20 + waveRadius);
            p.y = cy + Math.sin(p.angle) * (20 + waveRadius);
            // Reset when wave reaches edge
            if (waveRadius > 110) {
              p.angle = Math.random() * Math.PI * 2;
        ...
        // Pulsing radius
        const pulseRadius = p.baseRadius + Math.sin(time * 3 + p.angle) * 0.5;

        // Draw particle with glow
        const alpha = intensity * (0.4 + Math.sin(time * 2 + p.angle) * 0.2);
        ctx.beginPath();
        ...
        // Glow halo
        if (isActive && currentPhase !== 'disabled') {
          ctx.beginPath();
