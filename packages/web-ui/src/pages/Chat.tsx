    ...
    <>
      ...
      {tokens && (
        <div style={{ height: 2, background: '#1a1a1a', position: 'relative' }}>
          <div style={{ width: `${Math.min(pct, 100)}%`, height: '100%', background: '#666', transition: 'width .5s ease' }} />
        </div>
      )}
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <div className="chat-area">
            ...
            {messages.map((m, i) => (
              <div key={m.id ?? i} className={`chat-msg ${m.role === 'user' ? 'user' : m.role === 'tool' ? 'tool' : 'agent'} ${m.entered ? 'entered' : ''}`}>
            ...
            {thinking && (
              <div className="chat-msg agent">
                ...
                <div className="chat-bubble" style={{ display: 'flex', gap: 4, alignItems: 'center', padding: '12px 18px' }}>
                  <span style={{ width: 5, height: 5, borderRadius: '50%', background: '#666', animation: 'bounce 1.2s ease-in-out infinite' }} />
                  <span style={{ width: 5, height: 5, borderRadius: '50%', background: '#666', animation: 'bounce 1.2s ease-in-out infinite 0.2s' }} />
                  <span style={{ width: 5, height: 5, borderRadius: '50%', background: '#666', animation: 'bounce 1.2s ease-in-out infinite 0.4s' }} />
                </div>
              </div>
            )}
