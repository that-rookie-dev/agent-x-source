...
const CORE_TOOLS: ToolDefinition[] = [
  // ═══ FILESYSTEM ═══
  { id: 'file_read', name: 'Read File', description: 'Read the contents of a file', modelDescription: 'Read file contents with line-level paging. Use offset (0-based line number) and limit (number of lines) to read specific sections. Returns file content with line count metadata. Essential for examining code, config, logs, or data files.', category: 'filesystem', riskLevel: 'low', schema: { type: 'object', properties: { path: { type: 'string', description: 'File path relative to project root' }, offset: { type: 'number', description: 'Starting line number (0-based, default: 0)' }, limit: { type: 'number', description: 'Number of lines to return (default: all)' } }, required: ['path'] }, composable: true, source: 'builtin' },
  { id: 'file_write', name: 'Write File', description: 'Write content to a file', modelDescription: 'Write/create or append to a file. Use mode: "overwrite" (default) to replace the file, or "append" to add content at the end. Creates parent directories automatically.', category: 'filesystem', riskLevel: 'medium', schema: { type: 'object', properties: { path: { type: 'string', description: 'File path' }, content: { type: 'string', description: 'File content' }, mode: { type: 'string', description: 'Write mode: "overwrite" (default) or "append"' } }, required: ['path', 'content'] }, composable: true, source: 'builtin' },
  ...
  { id: 'folder_list', name: 'List Directory', description: 'List contents of a directory', modelDescription: 'List files and folders in a formatted table with type (d/f), name, size, and last modified date. Directories listed first, sorted alphabetically. Essential for exploring project structure.', category: 'filesystem', riskLevel: 'low', schema: { type: 'object', properties: { path: { type: 'string', description: 'Directory path (default: current directory)' } }, required: [] }, composable: true, source: 'builtin' },
  ...
  { id: 'delegate_to_crew', name: 'Delegate to Crew', description: 'Delegate a specific sub-task to an expert crew member', modelDescription: 'Delegate a clearly scoped sub-task to a specialized crew member by name or callsign. The crew runs in parallel and returns their result independently. Use for tasks that require deep domain expertise (DevOps, security, frontend, etc.). The crew name must match exactly — available crews are listed in the system.', category: 'agent_orchestration', riskLevel: 'medium', schema: { type: 'object', properties: { crew: { type: 'string', description: 'Crew member name or @callsign to delegate to (e.g. "Jordan Taylor" or "jordan_taylor")' }, task: { type: 'string', description: 'Specific, scoped task description for the crew to execute' } }, required: ['crew', 'task'] }, composable: true, source: 'builtin' },
