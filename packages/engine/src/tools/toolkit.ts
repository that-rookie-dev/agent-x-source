...
const CORE_TOOLS: ToolDefinition[] = [
  ...
  { id: 'render_channel_response', name: 'Render Channel Response', description: 'Render structured content blocks as native platform UI (Telegram inline keyboards, Slack Block Kit, Discord embeds/buttons)', modelDescription: 'On messaging channel sessions only. Use for responses that benefit from native UI: choices (inline buttons), status updates, structured data, dividers, code blocks with language hints. For simple conversational replies, use plain text — the bridge auto-formats it. Block types: text, code, header, list, quote, divider, status, table, choices, link, embed. choices block renders as native buttons (options[] with label, value, recommended, style, url). embed block renders as Discord embed, Slack section with fields, or Email callout (title, description, fields[], color, url).', category: 'agent_meta', riskLevel: 'low', schema: { type: 'object', properties: { blocks: { type: 'array', description: 'Content blocks to render. Each block has a "type" field: text (text), code (code, language), header (text, level), list (items, ordered), quote (text), divider (none), status (icon, text), table (headers, rows), choices (prompt, options[{label,value,recommended,style,url}], multi, placeholder), link (text, url), embed (title, description, fields[{name,value,inline}], color, url, footer).' } }, required: ['blocks'] }, composable: false, source: 'builtin' },
...
export function createDefaultToolkit(scopePath: string): { registry: ToolRegistry; executor: ToolExecutor } {
  ...
  executor.registerHandler('render_channel_response', renderChannelResponse.renderChannelResponse);
