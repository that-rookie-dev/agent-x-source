...
const CORE_TOOLS: ToolDefinition[] = [
  ...
  // ═══ WEB & NETWORK ═══
  { id: 'http_get', name: 'HTTP GET', description: 'Make GET request', modelDescription: 'Fetch URL content via GET. Alias: web_fetch. For rendered JS pages use web_browse.', category: 'web_network', riskLevel: 'medium', schema: { type: 'object', properties: { url: { type: 'string', description: 'URL' }, headers: { type: 'object', description: 'Custom headers' } }, required: ['url'] }, composable: true, source: 'builtin' },
  { id: 'web_fetch', name: 'Fetch URL', description: 'Fetch web page or API content', modelDescription: 'Fetch a URL and return body text/JSON. Read-only alias for http_get.', category: 'web_network', riskLevel: 'medium', schema: { type: 'object', properties: { url: { type: 'string', description: 'URL to fetch' }, headers: { type: 'object', description: 'Optional HTTP headers' } }, required: ['url'] }, composable: true, source: 'builtin' },
  ...
  { id: 'ask_clarification', name: 'Ask Clarifying Question', description: 'Ask the user a clarifying question before proceeding', modelDescription: 'Use when you need user input to proceed. For a single choice, provide options (user picks one). For multiple selections, set multiple:true with options. For structured data (names, dates, etc.), pass fields as an array of {key, label, placeholder?} objects. Keep options and fields minimal.', category: 'agent_meta', riskLevel: 'low', schema: { type: 'object', properties: { question: { type: 'string', description: 'The clarifying question to ask the user' }, options: { type: 'array', items: { type: 'string', description: 'A choice option' }, description: 'Choice strings for single or multiple selection' }, multiple: { type: 'boolean', description: 'If true, user may select multiple options (checkboxes). Default false (radio).' }, allowFreeform: { type: 'boolean', description: 'Whether to allow a custom text answer (default: true)' }, recommended: { type: 'string', description: 'Which option is recommended (must match an option value)' } }, required: ['question'] }, composable: false, source: 'builtin' },
